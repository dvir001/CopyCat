...
public class TtsVoiceService
{
    ...
    public async Task<GeneratedVoiceClip> GenerateClip(string voiceId, string text)
        => await GenerateClip(voiceId, text, compress: true);

    private async Task<GeneratedVoiceClip> GenerateClip(string voiceId, string text, bool compress)
    {
        ...
        if (voiceLower is "morshu" or "cabal")
            return await GeneratePythonClip(voice, text, compress);

        return await GeneratePiperClip(voice, text, compress);
    }
    ...
    {
        var sourceClips = new ConcurrentBag<GeneratedVoiceClip>();
        var scheduled = new List<(string Path, long StartMs)>();
        var consumed = new bool[lines.Count];

        try
        {
            using var generationLimit = new SemaphoreSlim(Math.Min(4, Math.Max(2, Environment.ProcessorCount)));
            async Task<(GeneratedVoiceClip Clip, long DurationMs)> GenerateSourceClip(string voiceId, string text)
            {
                await generationLimit.WaitAsync();
                try
                {
                    var clip = await GenerateClip(voiceId, text, compress: false);
                    sourceClips.Add(clip);
                    return (clip, ReadWaveDurationMs(clip.TempPath));
                }
                finally
                {
            ...
            var lineClipTasks = lines.Select(line =>
            {
                ...
                return GenerateSourceClip(voiceId, line.SpokenText);
            }).ToArray();
            var prefixDurationTasks = lines.Select(line =>
            {
                if (!voices.TryGetValue(line.Speaker, out var voiceId))
                    throw new PKError($"Speaker {line.Speaker} does not have a configured voice.");
                return line.CuePrefixes.Select(async prefixText =>
                    prefixText.Length == 0 ? 0L : (await GenerateSourceClip(voiceId, prefixText)).DurationMs).ToArray();
            }).ToArray();
            await Task.WhenAll(lineClipTasks.Cast<Task>().Concat(prefixDurationTasks.SelectMany(tasks => tasks)));
            var lineClips = lineClipTasks.Select(task => task.Result).ToArray();
            ...
            async Task<long> ScheduleLine(int lineIndex, long startMs)
            {
                ...
                var lineClip = lineClips[lineIndex];
                scheduled.Add((lineClip.Clip.TempPath, startMs));
                var endMs = startMs + lineClip.DurationMs;
                ...
                for (var fragmentIndex = 0; fragmentIndex < line.Fragments.Count; fragmentIndex++)
                {
                    ...
                    var cueOffsetMs = await prefixDurationTasks[lineIndex][cueIndex++];
    ...
    private static long ReadWaveDurationMs(string path)
    {
        using var stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read);
        using var reader = new BinaryReader(stream);
        if (stream.Length < 44 || new string(reader.ReadChars(4)) != "RIFF")
            throw new PKError("Generated voice clip is not a valid WAV file.");
        ...
        if (bytesPerSecond <= 0)
            throw new PKError("Generated voice clip has an invalid WAV header.");

        return Math.Max(1, (long)Math.Ceiling(dataBytes * 1000d / bytesPerSecond));
    }
    ...
    private async Task<GeneratedVoiceClip> GeneratePiperClip(string voiceId, string text, bool compress)
    {
    ...
    private async Task<GeneratedVoiceClip> GeneratePythonClip(string voiceId, string text, bool compress)
    {
        var outputPath = Path.Combine(Path.GetTempPath(), $"copycat-tts-{Guid.NewGuid():N}.wav");
        ...
        foreach (var python in pythonCandidates)
        {
            ...
            if (result.Success)
            {
                ...
                var finalPath = compress ? CompressToOggOpus(outputPath) : outputPath;
